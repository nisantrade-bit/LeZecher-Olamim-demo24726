...
import { Flame, Calendar, BookOpen, LayoutGrid, FileDown, Globe, Sparkles, AlertTriangle, Bell, Plus, X, CheckCircle2, Lock, ShieldCheck, LogOut } from 'lucide-react';
...
function MainAppContent() {
  ...
  // Admin Auth State & Security Controls
  ...
  // Verification helper for RPC public.is_admin()
  const checkIsAdminRPC = async (): Promise<boolean> => {
    try {
      if (!isSupabaseConfigured()) return false;
      const { data, error } = await supabase.rpc('is_admin');
      if (error) {
        console.warn('[Admin Auth] RPC is_admin check failed:', error.message);
        return false;
      }
      return data === true;
    } catch (err) {
      console.warn('[Admin Auth] Exception calling is_admin RPC:', err);
      return false;
    }
  };
    ...
    const initAuth = async () => {
      try {
        ...
        if (currentSession) {
          ...
          const adminResult = await checkIsAdminRPC();
          if (isMounted) {
        ...
        console.error('[Admin Auth] Session init error:', e);
        if (isMounted) {
          setSession(null);
          setUser(null);
          setIsAdmin(false);
  ...
  const handleAdminLogin = async (email: string, pass: string): Promise<{ success: boolean; errorMsg?: string }> => {
    ...
    try {
      ...
      if (error) {
        ...
        return { success: false, errorMsg: 'פרטי ההתחברות אינם נכונים.' };
      }

      if (!data.session) {
        setAuthSubmitting(false);
        return { success: false, errorMsg: 'לא ניתן להתחבר כעת. נסה שוב.' };
      }

      const adminResult = await checkIsAdminRPC();
      if (!adminResult) {
        setAuthSubmitting(false);
        return { success: false, errorMsg: 'המשתמש מחובר אך אינו מורשה כמנהל.' };
      }
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <header className="flex flex-col items-center justify-center border-b border-[#D8CFC0] pb-6 mb-8 gap-6 w-full text-center">
          ...
          {/* Language Selector & Admin Auth Control */}
          <div className="flex flex-col sm:flex-row items-center gap-3 mx-auto">
            ...
            <div className="flex items-center gap-2 font-sans">
              {isAdmin ? (
                <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-3 py-1.5 rounded-xl text-xs font-semibold text-amber-900 shadow-xs">
                  <ShieldCheck className="w-4 h-4 text-amber-600" />
                  <span>מצב מנהל</span>
                  <button
                    onClick={handleAdminLogout}
                    className="ms-1 px-2 py-0.5 bg-amber-200/60 hover:bg-amber-300/80 text-amber-950 text-[11px] font-bold rounded-md transition-all cursor-pointer flex items-center gap-1"
                    title="התנתקות"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#FFFDF8] hover:bg-[#FAF5EC] border border-[#D8CFC0] text-[#5D6D53] font-semibold text-xs rounded-xl transition-all shadow-xs cursor-pointer"
                >
