...
import { extractBaseHebrewLetters } from './transliteration';
...
-- Global Approved Name Pronunciation Dictionary Schema & Security
CREATE TABLE IF NOT EXISTS public.name_pronunciation_dictionary (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  canonical_name TEXT NOT NULL,
  canonical_name_normalized TEXT NOT NULL,
  pronunciation TEXT NOT NULL,
  transliteration_en TEXT,
  transliteration_ru TEXT,
  name_type TEXT NOT NULL CHECK (name_type IN ('deceased', 'father', 'mother')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT name_pron_dict_unique UNIQUE (canonical_name_normalized, name_type)
);

ALTER TABLE public.name_pronunciation_dictionary ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow public select on name_pronunciation_dictionary" ON public.name_pronunciation_dictionary;
CREATE POLICY "Allow public select on name_pronunciation_dictionary" ON public.name_pronunciation_dictionary FOR SELECT USING (true);

-- Secure First-Approval-Wins RPC Function (SECURITY DEFINER)
CREATE OR REPLACE FUNCTION public.upsert_approved_pronunciation(
  p_canonical_name TEXT,
  p_pronunciation TEXT,
  p_name_type TEXT,
  p_transliteration_en TEXT DEFAULT NULL,
  p_transliteration_ru TEXT DEFAULT NULL
)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_pronunciation IS NULL OR char_length(p_pronunciation) = 0 OR char_length(p_pronunciation) > 100 THEN
    RAISE EXCEPTION 'Invalid pronunciation length';
  END IF;

  IF p_canonical_name IS NULL OR char_length(p_canonical_name) = 0 THEN
    RAISE EXCEPTION 'Invalid canonical name';
  END IF;

  IF p_name_type NOT IN ('deceased', 'father', 'mother') THEN
    RAISE EXCEPTION 'Invalid name_type';
  END IF;

  -- Server-side canonical normalization matching extractBaseHebrewLetters
  v_normalized := regexp_replace(p_canonical_name, '[\u0591-\u05C7\s\d]', '', 'g');
  IF char_length(v_normalized) = 0 THEN
    v_normalized := p_canonical_name;
  END IF;

  -- First Approval Wins: ON CONFLICT DO NOTHING
  INSERT INTO public.name_pronunciation_dictionary (
    canonical_name,
  ...
  VALUES (
    ...
    v_normalized,
    p_pronunciation,
    p_transliteration_en,
    p_transliteration_ru,
    p_name_type,
    NOW()
  )
  ON CONFLICT (canonical_name_normalized, name_type)
  DO NOTHING
...
GRANT EXECUTE ON FUNCTION public.upsert_approved_pronunciation TO anon, authenticated, service_role;
`.trim();
...
/**
 * Attempts to insert an approved pronunciation into the global dictionary table.
 * Adheres strictly to "First Approval Wins" (ON CONFLICT DO NOTHING).
 * If name_pronunciation_dictionary already has an entry for (normalized_name, name_type), it is NOT overwritten.
 */
export async function upsertApprovedPronunciation(
  canonicalName: string,
  pronunciation: string,
  nameType: 'deceased' | 'father' | 'mother',
  transliterationEn?: string,
  transliterationRu?: string
): Promise<{ status: 'inserted' | 'already_exists' | 'error'; error?: any }> {
  if (!canonicalName || !pronunciation || !nameType) {
    return { status: 'error', error: 'Missing required parameters' };
  }

  const normalized = extractBaseHebrewLetters(canonicalName);
  if (!normalized) {
    return { status: 'error', error: 'Invalid normalized name' };
  }

  try {
    // 1. Try secure RPC first
    const { data: rpcData, error: rpcError } = await supabase.rpc('upsert_approved_pronunciation', {
      p_canonical_name: canonicalName,
      p_pronunciation: pronunciation,
      p_name_type: nameType,
      p_transliteration_en: transliterationEn || null,
      p_transliteration_ru: transliterationRu || null
    });

    if (!rpcError && rpcData) {
      return { status: rpcData === 'inserted' ? 'inserted' : 'already_exists' };
    }

    // 2. Client-side fallback if RPC is not deployed yet on backend
    const { data: existing } = await supabase
      .from('name_pronunciation_dictionary' as any)
      .select('id')
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        transliteration_ru: transliterationRu || null
      });
    ...
    console.warn('[upsertApprovedPronunciation exception]', err);
    return { status: 'error', error: err };
  }
}

/**
 * Fetches an approved vocalized pronunciation from the global dictionary table.
 * Returns null if no approved pronunciation exists for the normalized name and name_type.
 */
export async function fetchApprovedPronunciation(
  rawName: string,
  nameType: 'deceased' | 'father' | 'mother'
): Promise<string | null> {
  ...
  const normalized = extractBaseHebrewLetters(rawName);
  if (!normalized) {
    return null;
  }
  ...
  try {
    ...
    if (!error && data && data.pronunciation) {
