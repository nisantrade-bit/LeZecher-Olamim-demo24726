...
Return a JSON object with a single string property "refinedNotes".`;
  ...
  app.get('/api/deceased/:id', (req, res) => {
    try {
      initDatabaseFiles();
      ...
      const record = currentData.find((item: any) => Number(item.id) === Number(id));
      if (record) {
      ...
      console.error("Error reading single deceased record:", error);
      res.status(500).json({ error: "Failed to read database" });
    }
  });
  ...
  app.get(['/', '/index.html', '/share/:id', '/share/:id.html', '/m/:id', '/p/:id', '/deceased/:id', '/memorial/:id', '/card/:id', '/yahrzeit/:id', '/m/:id.html', '/p/:id.html', '/deceased/:id.html', '/memorial/:id.html'], async (req, res, next) => {
    try {
      ...
      if (!deceased && dataParamStr) {
        try {
          const dataStr = String(dataParamStr).trim();
          ...
          if (parsed && (parsed.i || parsed.id) && (parsed.n || parsed.name)) {
            deceased = {
              ...
              ageAtDeath: parsed.a !== undefined ? Number(parsed.a) : undefined,
              birthDate: parsed.bd ? String(parsed.bd) : undefined
            };
            // Automatically persist record to server database
            if (!db.some((item: any) => Number(item.id) === Number(deceased.id))) {
              db.push(deceased);
              fs.writeFileSync(DATABASE_FILE, JSON.stringify(db, null, 2), 'utf-8');
            }
          }
        } catch (e) {
          console.error("Error decoding server query data payload:", e);
        }
      }
